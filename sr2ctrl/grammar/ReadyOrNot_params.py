...
# ※ 実際のチェック順序は「ReadyOrNot.py」に定義されています。このファイル「ReadyOrNot_params.py」の中で並べ替えても処理には反映されません。
...
# コマンドメニューだけ開く（主にナンバーオーダーに繋げて使う想定）
kw_open_cmd = {
	"base": (
		"オーダー",
		"ボーダー",
...
# ナンバーオーダー用
# 英語読みは他と競合しやすいので非推奨（"フォー"と"フォーメーション"など）
kw_number = {
	"base": ("ばん", "番", "盤", "版", r"取[り]*消", "訂正",),
	# ↑"バン"はフラッシュバンと競合するので不使用
	# ↑"back"と同じ中身も入れておくこと
	"1": ("いち", r"[一壱Ⅰⅰ①１1]",),
	"2": ("に", r"[二弐Ⅱⅱ②２2]",),
	"3": ("さん", r"[三参Ⅲⅲ③３3]",),
	...
	"7": ("なな", "ナナ", r"[七Ⅶⅶ⑦７7]",),
	"8": ("はち", "ハチ", r"[八Ⅷⅷ⑧８8]",),
	"9": ("きゅう", "キュウ", r"[九旧吸Ⅸⅸ⑨９9]",),
	"0": ("ぜろ", "ゼロ", "zero", "Zero", "ZERO", r"[〇零０0]",),
	"back": (r"取[り]*消", "訂正",), # １階層戻る
}
...
kw_so_control = {
	...
	# ブリーチングツール／グレネードに使う言葉は、kw_breach_tools／kw_grenadesを使用する
}
# ステップオーダー中、何も発話せずこの時間経過したら自動で中断するタイムアウト秒数
so_timeout = 15
...
kw_grenades = {
	...
	"none": ( # グレネード無し（ステップオーダー中のみ使用）
		"行け", # ホールド命令実行との区別は特別に不要
		"いけ",
		"池",
		"やれ",
		"入れ",
